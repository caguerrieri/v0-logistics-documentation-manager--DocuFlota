...
export function VehicleDocumentsModal({ isOpen, onClose, vehicleId, vehiclePlate }: VehicleDocumentsModalProps) {
  ...
  const fetchDocuments = async () => {
    ...
    setLoading(true)
    try {
      const response = await fetch(`/api/documents?category=vehicle&entity_id=${vehicleId}`)
      if (response.ok) {
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Documentos del Vehículo {vehiclePlate}
          </DialogTitle>
          <DialogDescription>Lista completa de documentos asociados a este vehículo</DialogDescription>
        </DialogHeader>
